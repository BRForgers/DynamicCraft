...
public class GuiMagicCollector extends GuiContainer{
	...
	public GuiMagicCollector(EntityPlayer player, InventoryPlayer inv, ItemStack stack, InventoryMagicCollector inventoryItem) {
		...
		if (ItemHelper.getString(stack, "biome", "").contains(BiomeGenBase.extremeHills.biomeName) ||
				ItemHelper.getString(stack, "biome", "").contains(BiomeGenBase.swampland.biomeName)) {
			porc = 25;
		}else {
			porc = 5;
		}
